...
-- and mirror it to NavDataMemory.lua for full DCS session persistence.
...
local nav_data_memory_path = LockOn_Options.script_path .. "Systems/NavDataMemory.lua"
...
local function load_nav_unit_state_from_file()
    ...
    if not ok or type(data) ~= "table" or type(data.state) ~= "table" then
        return false
    end

    nav_unit_persistence.state = data.state
    return true
end
...
local function write_nav_unit_state_to_file(state)
    ...
    file:write("-- SK60 NAV unit persistent memory. Generated by nav_system.lua.\n")
    file:write("-- This file stores RNAV/NAV programming between DCS sessions.\n")
    file:write("return ")
    file:write(serialize_lua_value({ state = state }))
    file:write("\n")
    file:close()
...
local function copy_waypoint_slot(slot)
    slot = type(slot) == "table" and slot or {}
    return {
        vor_frequency_mhz = normalize_vor_frequency(sanitize_number(slot.vor_frequency_mhz, 113.00, vor_min_frequency_mhz, vor_max_frequency_mhz)),
        offset_radial_deg = normalize_radial(sanitize_number(slot.offset_radial_deg, 0)),
        offset_distance_nm = clamp(sanitize_number(slot.offset_distance_nm, 0, 0, 999.9), 0, 999.9),
    }
end
